...
CREATE TABLE IF NOT EXISTS `rsa` (
  ...
  `encryptedKey` longtext DEFAULT NULL,
  `timestamp` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`)
...
-- Dumping structure for table sp_chat_trial.rsa_aes
CREATE TABLE IF NOT EXISTS `rsa_aes` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`ID`),
  KEY `FK_rsa_aes_session` (`idSession`),
  CONSTRAINT `FK_rsa_aes_session` FOREIGN KEY (`idSession`) REFERENCES `session` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
